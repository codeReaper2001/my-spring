...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory implements AutowireCapableBeanFactory {
    ...
    protected Object doCreateBean(String beanName, BeanDefinition beanDefinition, Object[] args) {
        ...
        try {
            ...
            if (beanDefinition.isSingleton()) {
                ...
                // 此回调函数的功能是，调用获取bean时会用InstantiationAwareBeanPostProcessor
                // 这个后置处理器将bean封装成代理对象后返回
    ...
    protected void applyBeanPostProcessorsBeforeApplyingPropertyValues(String beanName, Object bean, BeanDefinition beanDefinition) {
        for (BeanPostProcessor beanPostProcessor : getBeanPostProcessors()) {
            if (beanPostProcessor instanceof InstantiationAwareBeanPostProcessor) {
                ...
                if (null != pvs) {
                    // 使用修改后得到的pvs来覆盖原来的BeanDefinition中的内容
    ...
    protected Object applyBeanPostProcessorsBeforeInstantiation(Class<?> beanClass, String beanName) {
        for (BeanPostProcessor beanPostProcessor : getBeanPostProcessors()) {
            if (beanPostProcessor instanceof InstantiationAwareBeanPostProcessor) {
                InstantiationAwareBeanPostProcessor iaBeanPostProcessor = (InstantiationAwareBeanPostProcessor) beanPostProcessor;
                Object result = iaBeanPostProcessor.postProcessBeforeInstantiation(beanClass, beanName);
